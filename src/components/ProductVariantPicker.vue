<template>
	<div class="variant-picker">
		...
		<div
			v-if="variants.length === 1 && productOptions.length === 0 && variants[0].title && variants[0].title !== 'Default' && variants[0].title !== 'Default Title' && variants[0].title.toLowerCase() !== 'default'"
			...
			>
				...
				>
					<button
						...
						@mouseenter="
							handleColorSwatchHover($event, value, true)
						"
						@mouseleave="
							handleColorSwatchHover($event, value, false)
						"
					>
						<span
							style="
...
<script setup>
	...
	onMounted(async () => {
		...
		if (variants.value.length > 0) {
			...
			if (!variantToSelect) {
				variantToSelect =
					variants.value.find((v) => v.available) ||
					variants.value[0];
			}
	...
	const handleColorSwatchHover = (event, value, isHovering) => {
		...
		if (isHovering) {
			event.target.style.transform = isSelected
				? 'scale(1.1)'
				: 'scale(1.05)';
